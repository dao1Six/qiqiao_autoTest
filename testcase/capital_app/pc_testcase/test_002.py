...
【补丁】——PC端，流程中打印按钮，系统报错'''
        ...
        time.sleep(3)
        formPage.switch_tab(2)
        self.driver.close()
        formPage.switch_tab(1)
    ...
    def test_02( self ):
        '''【运行平台】——资产管理应用，多表关联配置数据联动筛选，新建的按钮生效，旧按钮不生效（需要对旧数据验证，全面走查外键联动筛选）'''
        portalPage = PortalPage(self.driver)
        portalPage.PortalPage_Click_HeaderMenu("应用")
        applicationListPage = ApplicationListPage(self.driver)
        applicationListPage.ApplicationListPage_ClickApplicationIcon('默认分组','资产管理')
        businessPage = BusinessPage(self.driver)
        businessPage.BusinessPage_LeftMenu_Click("调拨管理")
        time.sleep(1)
        businessPage.BusinessPage_LeftMenu_Click("资产调拨")
        businessPage.ListComponent_Click_ListHeader_Button("添加1")
        formPage = FormPage(self.driver)
        time.sleep(2)
        formPage.ForeignSelection_Sendkeys("调出仓库","广州",1)
        time.sleep(2)
        formPage.MultiForm_BatchManagementButton_Click("调拨明细")
        self.assertEqual("广州",formPage.MultiForm_RelationForm_GetTdValue("调拨明细",1,1))
        formPage.MultiForm_BathManagePage_ConfirmButton_Tick("调拨明细")
        time.sleep(2)
        formPage.ForeignSelection_Sendkeys("调出仓库","江门",1)
        formPage.MultiForm_BatchManagementButton_Click("调拨明细")
        self.assertIsNone(formPage.MultiForm_RelationForm_GetTdValue("调拨明细",1,1))
